...
export const api = {
  ...
  trace: (id: string) => request<Evidence>(`/incidents/${id}/tools/trace`, { method: "POST", body: JSON.stringify({ service: "order-service" }) }),
  change: (id: string) => request<Evidence>(`/incidents/${id}/tools/change`, { method: "POST", body: JSON.stringify({ service: "inventory-service" }) }),
  autoCollect: (id: string) => request<Evidence[]>(`/incidents/${id}/tools/auto-collect`, { method: "POST" }),
